...
(Read `.myfootmarks/current-trip` → slug → `Trips/<slug>/trip.yaml`. Extract `destination`, `start_date`, `end_date`, `regions`, `travelers`. Bail to `/myfootmarks:intake` if missing.)
