...
(Read `.myfootmarks/current-trip` → slug → `<slug>/trip.yaml`. Extract `destination`, `start_date`, `end_date`, `regions`, `travelers`, `budget_tier`. Bail to `/myfootmarks:intake` if missing.)
