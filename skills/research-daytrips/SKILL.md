---
name: research-daytrips
description: Research day-trip regions near your home base
---

# Research Daytrips

Run live web research on day-trip-worthy regions surrounding the active trip's destination — places worth a dedicated day out from the home base. Write findings to `research/daytrips.md` in reference-family format (structured list of regions, not a per-activity list).
...
`.myfootmarks/trips/<slug>/research/daytrips.md` — markdown document with YAML frontmatter carrying a structured `places:` array plus the human-readable body.
...
```
You are researching day-trip regions for a trip.

USE WEBSEARCH AND WEBFETCH FOR LIVE DATA. Ferry schedules, park passes, and seasonal access change — verify with current sources.
...
Find 5–8 day-trip regions worth dedicating a full day to, reachable from the home base. Each region should offer enough to fill a day (multiple activities, meal opportunities, reasonable return time).

Return your findings as TWO blocks: a structured `places:` YAML array AND a human-readable Markdown body. Each region is a `places:` entry (use a representative lat/lon — e.g., the region's main town or anchor attraction).
...
Render the same regions as Markdown in REFERENCE-FAMILY format — structured list of regions (not a per-activity list):

## Day-trip regions

### <Region name>

- **Distance / time from home base:** <e.g., "45 min drive", "1h10 via ferry + walk", "90 min train">
- **Getting there:** <car, ferry, train, bus; note any must-book-ahead logistics>
- **What's notable:** <three to five bullet-points of the most notable activities, sights, or experiences in this region — not individual venues, just anchoring highlights>
- **Best for travelers with interests in:** <match to trip interests — e.g., "outdoor, scenic, food">
- **Good day length:** <e.g., "6-8 hours including travel; feasible as a half-day if limited">
- **Notes:** <seasonal considerations, kid-friendliness, cost range>
- **Source:** <markdown link>

Prioritize regions that:
1. Are reachable in 1–2 hours one-way
2. Offer enough variety to fill a day
3. Contrast with the home-base region (different feel, different activities)
4. Match the travelers' interests

Avoid: regions under 30 minutes (too close — essentially extensions of the home base), regions over 2.5 hours one-way (too much travel for a day trip).
```

### Step 4: Render output

Write `.myfootmarks/trips/<slug>/research/daytrips.md` with frontmatter that includes both metadata and the `places:` array from the subagent:
...
places:
  - id: <slug>
    ...
    address_street: <string | null>
  # …one entry per daytrip destination in the markdown body, same order
---
```

Then the subagent's structured Markdown body.

### Step 5: Log the run

```json
{"timestamp": "<ISO 8601>", "skill": "research-daytrips", "slug": "<slug>", "status": "ok", "consumed_inputs": ["trip.yaml"], "outputs_written": [".myfootmarks/trips/<slug>/research/daytrips.md"]}
```
